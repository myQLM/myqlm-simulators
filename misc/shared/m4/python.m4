...
AC_ARG_WITH([python-prefix], [AS_HELP_STRING([--with-python-prefix], [Change the default python prefix path])], [pc_cv_python_site_dir="$withval/python$PYTHON_VERSION/site-packages"],
[
...
if interpreter.startswith('python'):
    sitedir = site.getsitepackages()[[0]][[len(sys.prefix) + 1:]]
else:
    sitepkg = site.getsitepackages()[[0]]
    if sitepkg.startswith(libdir):
        sitedir = sitepkg[[len(os.path.dirname(prefix)) + 1:]]
], [dnl
    sys.stdout.write(sitedir)
])], [pc_cv_python_site_dir=`./conftest $prefix $libdir`],
     [AC_MSG_FAILURE([failed to run Python program])])
     AC_LANG_POP(Python)[]dnl
       pc__strip_prefix=`echo "$pc_cv_python_prefix" | sed 's|.|.|g'`
...
AC_ARG_WITH([python-execprefix], [AS_HELP_STRING([--with-python-execprefix], [Change the default python executable prefix path])], [pc_cv_python_exec_dir="$withval/python$PYTHON_VERSION/site-packages"],
[
