...
AC_DEFUN([AGENT_NETSNMP_CHECK_CONFIG],
[
  _libnetsnmp_config="no"

  AC_ARG_WITH(agent-net-snmp,
[
What SNMP package do you want to use (please select only one):
AC_HELP_STRING([--with-agent-net-snmp@<:@=ARG@:>@],
[NET-SNMP is mandatory, optionally specify path to net-snmp-config])
...
_full_libnetsnmp_libs="`$_libnetsnmp_config --agent-libs`"
