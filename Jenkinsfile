...
node("any||$BRANCH_NAME") {
    try {
        ...
        sh '''
        cd $WORKSPACE
        rpm -i --force --nodeps --dbpath /tmp/rpmsdb/${JOB_NAME/\\//-}-rpms --relocate /usr=/$PWD/rpm_documentation/ --relocate /etc=$PWD/rpm_documentation/etc archives/${JOB_NAME/\\/*}-doc*x86_64.rpm
        '''
        }
        ...
        def htmldir = sh returnStdout: true, script: 'ls -d rpm_documentation/share/doc/${JOB_NAME/\\/*}/*/'
        publishHTML([allowMissing: true, alwaysLinkToLastBuild: false, keepAll: false, reportDir: htmldir, reportFiles: 'index.html', reportName: 'Doxygen reports', reportTitles: ''])
